...
router.get("/", async (req, res, next) => {
  try {
    res.json(
      (await populated(Cart.findOne({ user: req.user._id }))) || {
        user: req.user._id,
        items: [],
      },
    );
...
router.put("/:productId", async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20)
      return res
        .status(422)
        .json({ message: "Quantity must be between 1 and 20" });
    ...
    const item = cart.items.find(
      (entry) => String(entry.product) === req.params.productId,
    );
    if (item) item.quantity = quantity;
    else cart.items.push({ product: req.params.productId, quantity });
    await cart.save();
...
router.delete("/:productId", async (req, res, next) => {
  try {
    ...
    res.json(
      cart
        ? await populated(Cart.findById(cart._id))
